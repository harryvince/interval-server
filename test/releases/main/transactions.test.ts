...
import { config, consoleUrl, isAppendUIEnabled, sleep } from '../../_setup'
...
test.describe
  .parallel('Basic transactions tests', () => {
    ...
    test('io.display.link', async ({ page, transactions }) => {
      ...
      await expect(
        page.locator('a:has-text("Link to action")')
      ).toHaveAttribute('href', actionUrl)
    ...
    test('io.display.metadata', async ({ page, transactions }) => {
      ...
      for (let i = 0; i < headings.length; i++) {
        ...
        await expect(
          page.locator('dt:has-text("Is true")').nth(n)
        ).toBeVisible()
        await expect(page.locator('dd:has-text("true")').nth(n)).toBeVisible()
        await expect(
          page.locator('dt:has-text("Is false")').nth(n)
        ).toBeVisible()
        await expect(page.locator('dd:has-text("false")').nth(n)).toBeVisible()
        await expect(
          page.locator('dt:has-text("Is null")').nth(n)
        ).toBeVisible()
        await expect(
          page.locator('dt:has-text("Is empty")').nth(n)
        ).toBeVisible()
        await expect(
    ...
    test('io.display.html', async ({ page, transactions }) => {
      ...
      await expect(
        page.locator('.prose h1:has-text("Heading 1")')
      ).toBeVisible()
    ...
    test.describe('io.input.slider', () => {
      ...
      test('Keyboard entry', async ({ page, transactions }) => {
        ...
        await transactions.continue()
        await transactions.expectValidationError('This field is required.')
    ...
    test('io.select.multiple', async ({ page, transactions }) => {
      ...
      await page
        .locator(`input[type="checkbox"][value="${date}"]`)
        .last()
        .click()
    ...
    test.describe('Search', () => {
      ...
      test('Two searches', async ({ page, transactions }) => {
        ...
        await expect(
          page.locator(
            '[data-pw-search-result]:has-text("Viewer"):nth-child(1)'
          )
        ).toBeVisible()
    ...
    test.describe('Unlisted', () => {
      ...
      test('Navigation', async ({ page }) => {
        await page.goto(`${await consoleUrl()}/unlisted_page`)
        await expect(page.locator('h2:has-text("Unlisted page")')).toBeVisible()
        await expect(page.locator('a:has-text("unlisted_listed")')).toHaveCount(
          2
        )
        await expect(page.locator('a:has-text("Listed subpage")')).toHaveCount(
          2
        )
        await expect(
        ...
        await expect(
          page.locator('a:has-text("Unlisted subpage")')
        ).toHaveCount(0)
      })
    })
  })
