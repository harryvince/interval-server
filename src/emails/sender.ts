...
import * as emailClient from '~/clients/email'
import env from '~/env'
import { logger } from '~/server/utils/logger'

const TEMPLATES_FOLDER = path.join(__dirname, '..', '..', 'email-templates')
...
export default function emailSender<T extends TemplateData>(
  ...
  return async function sendEmail(
    ...
    logger.info('✉ Mailer: sending email to', to)
    ...
    const response = await emailClient.sendEmail(message)
