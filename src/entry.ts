...
program
  ...
  .addCommand(new Command("init").description("initializes Interval Server"))
...
async function main() {
  ...
  if (cmd === "start") {
    ...
    if (process.env.NODE_ENV === "production") await setupDb();
    ...
    server.listen(Number(envVars.PORT), () => {
      logger.info(`📡 Interval Server listening at ${envVars.APP_URL}`);
      logger.info(
